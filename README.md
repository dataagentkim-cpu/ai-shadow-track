...
- `decision_job` (화요일 07:00 KST, 장 시작 전) — 월요일 종가+뉴스 기준으로 `run_weekly.run_decision()` 실행, 판단만 로그
- `execution_job` (화요일 09:05 KST, 장 시작 후) — 실제 당일(화요일) 시가 기준으로 `run_weekly.run_execution()` 실행, 슬리피지+수수료+거래세 반영해 체결/벤치마크 확정
  - `config.EXECUTION_SLIPPAGE_PCT` (0.3%, 진입 시), `config.BROKER_FEE_PCT` (0.015%, 매수/매도 각각), `config.TRANSACTION_TAX_PCT` (0.18%, 매도 시만 — KOSPI/KOSDAQ 혼합 근사치)
