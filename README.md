...
개인용 섀도우 트랙. 기획 원본은 `shadow-track-build-brief.md`.
...
- `DART_API_KEY` — 다른 로컬 프로젝트(dart-ma-screener)에 있던 기존 키 재사용, 배당 공시 조회용
...
- **재무 공시**: 지금은 스크리너/LLM 판단에 재무제표 데이터를 아예 안 쓰고 있어서 PIT 필터링 대상 자체가 없다 (배당 데이터만 DART에서 가져오며, `dart_client._safe_fiscal_year()`가 4월 이전엔 전전년도까지 내려가는 방식으로 보수적으로 PIT를 지킨다).
...
- `src/dart_client.py` — DART 공시에서 종목별 현금배당수익률 조회
