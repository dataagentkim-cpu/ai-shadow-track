...
조정하는 stateful 판단을 로그로 쌓고, 내 실제 보유·지수·동일가중 baseline과 **②a가치/②b모멘텀/②c퀄리티/
②d자유형 4렌즈 병렬**로 비교하는 개인용 섀도우 트랙. **기준 설계 문서는 `SPEC.md`** (원본
`shadow-track-build-brief.md`의 확장판 — 코드와 문서가 어긋나면 SPEC.md가 우선, 바꾸려면 먼저 확인받을 것).

**진행 상태**: SPEC.md가 정의한 4렌즈 병렬 확장(가치·퀄리티 렌즈가 실제 재무 지표 PER/PBR/ROE/부채비율을
인풋으로 받는 것 포함)이 완료·실전 통합 테스트까지 마쳤다 (`llm_judge.py`/`run_weekly.py`/`benchmark.py`/
`risk_metrics.py`/`telegram_bot.py` 전 파이프라인 재배선 — 자세한 건 `context-notes.md` "4렌즈 확장 재개 +
실전 배선 완료" 항목 참조).

**②a~②d 4렌즈 + 3개 baseline**: ① 내 실제 보유(고정) / ②a~d AI 4렌즈 판단(각자 독립적으로 stateful, 공통
shortlist 공유) / ③ 코스피·코스닥 지수 / ④ 그 주 스크리너 shortlist를 LLM 판단 없이 동일가중 보유하는
baseline. **핵심 질문은 각 렌즈(②x)가 ④를 이기는가** — 못 이기면 "그 렌즈의 LLM 판단이 값을 못 하는 것"이고,
스크리너가 후보군을 좁혀준 것 이상의 부가가치를 냈다고 볼 수 없다는 뜻이다.

**AI 판단 방식**: 매주 shortlist·뉴스와 함께 **각 렌즈 자신의 지난주 목표 포트폴리오**(매수가·수익률은 비공개)를
참고해 "오늘 이 가격에 당신의 렌즈 기준으로 새로 산다면 살 것인가"를 기준으로 재평가한다 — 앵커링·처분효과를
막기 위해서다. "전량 유지(무거래)"도 완전히 정상적인 선택지다. 4개 렌즈는 서로의 포트폴리오를 참고하지 않고
완전히 독립적으로 판단하며(블라인드 격리), 자유형(②d)만 매주 `weekly_perspective` 필드에 그 주 선택한 관점을
선언한다. **내 실제 보유(①)는 이 판단 과정에 절대 들어가지 않는다** (블라인드 유지,
`run_weekly._get_previous_lens_portfolio()`가 각 렌즈의 `track_id`로만 하드 필터링).
...
## 텔레그램 명령어
- `/performance` — 전 트랙(내 보유/②a~d 4렌즈/지수/동일가중) 최신 수익률
- `/latest` — 이번 주 4렌즈 판단 전체 (렌즈별로 메시지 분리, 자유형은 그 주 관점 선언 포함)
- `/history` — 최근 수익률 추이 (전 트랙)
- `/why 종목명` — 특정 종목의 최근 판단 이유 (어느 렌즈에서 나온 판단인지 라벨 포함)
- `/holdings` — 내 실제 보유 스냅샷
- `/alpha` — 각 렌즈(②a~d)가 동일가중 baseline(④)을 이겼는지, 주간·누적 스프레드
...
- `execution_job` (화요일 09:05 KST, 장 시작 후) — `run_weekly.run_execution()` 실행, 실제 당일 시가 기준으로 체결/벤치마크 확정
  ...
  - 내 보유(①)·4렌즈(②a~d)·지수(③)·동일가중 baseline(④) 전 트랙 모두 `benchmark._get_open_price()` 하나로 통일해 같은 화요일 시가 기준에서 출발한다. 그 날짜 데이터가 없으면(장 시작 전 오호출 등) 조용히 넘어가지 않고 바로 에러를 내고, 보유 중 거래정지/상장폐지가 감지되면 `_get_open_price_safe()`가 마지막 시가로 동결 청산 처리한다.
  - ④는 그 주 스크리너 shortlist 전체를 LLM 판단 없이 동일가중으로 보유하는 baseline — 각 렌즈(②a~d)와 완전히 같은 체결/비용 규칙을 공유해서(`benchmark._snapshot_rebalanced_track`) ②x−④ 스프레드가 순수하게 "그 렌즈 LLM 판단의 기여도"만 반영하게 했다.
...
- **재무 공시**: `dart_client._safe_fiscal_year()`가 사업보고서 법정 제출기한(3월 말)을 감안해 아직 공시 안 됐을 연도는 건너뛴다 — 배당수익률·재무제표(ROE/부채비율/영업이익률/PER/PBR) 조회 둘 다 이 안전장치를 공유한다. 재무제표 값은 `screener_output`에 기록됨과 동시에 `run_weekly.run_decision()`이 shortlist 종목별로 결합해 4렌즈 LLM 판단 인풋(특히 가치·퀄리티 렌즈)으로도 그대로 전달된다.
...
- `src/llm_judge.py` — ②a가치/②b모멘텀/②c퀄리티/②d자유형 4렌즈 병렬 stateful 블라인드 LLM 판단
- `src/benchmark.py` — 전 트랙(①+4렌즈+③+④) 총수익 스냅샷 (배당·거래비용·PIT 가격 전부 반영)
